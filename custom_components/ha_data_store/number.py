...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    try: be = get_bridge_entities_for_platform(hass, "number", bdi)
    except Exception as e: _LOGGER.error("[bridge] number: %s", e); return
    if not be: return
    entities = [ent for _, ent in be]; reg = er.async_get(hass)
    for eid, ent in be: reg.async_get_or_create(domain="number", platform=DOMAIN, unique_id=ent.unique_id, suggested_object_id=eid.split(".", 1)[1])
